...
# 📁 파일 경로 설정
INPUT_DATA_FILE = "설문조사_전처리데이터_20250620_0731_processed.xlsx"  # 입력 데이터 파일
OUTPUT_HTML_FILE = "서울아산병원 협업평가 대시보드.html"  # 출력 HTML 파일

# 📊 데이터 컬럼 정의 (데이터 구조 변경 시 수정)
EXCEL_COLUMNS = [
...
def get_data_summary(df):
    ...
    return {
        "총_응답수": len(df),
        "연도별_응답수": df['설문연도'].value_counts().to_dict(),
        "부문별_응답수": df['피평가부문'].value_counts().to_dict(),
        "평균_종합점수": df['종합 점수'].mean().round(2),
        "데이터_기간": f"{df['설문연도'].min()}년 ~ {df['설문연도'].max()}년"
    }
...
def main():
    ...
    try:
        ...
        print(f"📈 평균 종합 점수: {summary['평균_종합점수']}점")
